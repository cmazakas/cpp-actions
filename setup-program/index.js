    ...
    if (package_match !== null) {
      ...
      if (apt_get_exit_code !== 0) {
        ...
        for (const install_match of install_matches) {
          if (isSudoRequired()) {
            apt_get_exit_code = await exec.exec(`DEBIAN_FRONTEND=noninteractive TZ=Etc/UTC sudo -n apt-get install -f -y --allow-downgrades ${install_match}`, [], opts)
          } else {
            apt_get_exit_code = await exec.exec(`DEBIAN_FRONTEND=noninteractive TZ=Etc/UTC apt-get install -f -y --allow-downgrades ${install_match}`, [], opts)
          }
          if (apt_get_exit_code === 0) {
            break
          }
